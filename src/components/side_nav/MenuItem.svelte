...
<div class="{type === 'parent' ? 'menu-item' : 'menu-sub-item'} block select-none">
  <span
  ...
  >
    <span
      class="icon--route-name flex flex-1 items-center select-none transform {hovering ? 'translate-x-2' : 'translate-x-0'} transition-all duration-300 ease-linear"
    >
      <span class=" icon">
    ...
    <div class=" menu-sub-items select-none  {expand && full?'block ':'hidden'}  transition-display duration-300 ease-linear" >
      <slot />
    </div>
</div>
