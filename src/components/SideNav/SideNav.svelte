<script>
  ...
  import {APP_WIDTH, MENU_WIDTH, MARGIN_LEFT, MINI_MENU } from '../../main_store.ts';
  let open = true;
  let hovering = false;
let oldWidth = '75'
...
<div
  ...
  lg:flex sm:block  transition-all  duration-300 ease-in-out"
  on:mouseover="{e => {
    hovering = true;
...
<slot name="overlay">
  <div
    ...
    lg:{$MINI_MENU && hovering ? 'block' : 'hidden'} w-full max-h-full transition-all  duration-300 ease-in-out"
    on:click="{() => {
      open = !open;
      hovering = false;
...
<div class="px-4 pt-1 lg:px-6 main-content lg:ml-{$MARGIN_LEFT} transition-all  duration-300 ease-in-out">
  <slot name="app-bar">
